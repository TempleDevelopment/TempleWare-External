...
bool ImGui::CustomSlider(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    // line
    const float frame_center = frame_bb.Min.y + ((frame_bb.Max.y - frame_bb.Min.y) / 2.f);
    window->DrawList->AddLine(ImVec2(frame_bb.Min.x + style.ItemInnerSpacing.x, frame_center), ImVec2(frame_bb.Max.x, frame_center), frame_col, 1.f);

    // circle
    if (grab_bb.Max.x > grab_bb.Min.x)
    {
        float center = grab_bb.Min.y + (grab_bb.Max.y - grab_bb.Min.y) / 2.f;
        window->DrawList->AddLine(ImVec2(frame_bb.Min.x, center), ImVec2(grab_bb.Max.x, center), GetColorU32(ImGuiCol_SliderGrab), 1.f);
        window->DrawList->AddCircleFilled(ImVec2(grab_bb.Min.x + ((grab_bb.Max.x - grab_bb.Min.x) / 2.f) + (style.ItemInnerSpacing.x / 1.5f), center), (grab_bb.Max.y - grab_bb.Min.y) / 2.5f, ImColor(1.f,1.f,1.f,1.f), 0);
    }
   ...
   // if (label_size.x > 0.0f)
    RenderText(ImVec2(frame_bb.Max.x + (style.ItemInnerSpacing.x * 2.5f), frame_bb.Min.y + style.FramePadding.y), value_buf);
...
bool ImGui::CustomSliderFloat(const char* title, const char* label, float* v, float v_min, float v_max, const char* format, ImGuiSliderFlags flags)
{
    Text(title);
    return CustomSlider(label, ImGuiDataType_Float, v, &v_min, &v_max, format, flags);
}

bool ImGui::CustomSliderInt(const char* title, const char* label, int* v, int v_min, int v_max, const char* format, ImGuiSliderFlags flags)
{
    Text(title);
    return CustomSlider(label, ImGuiDataType_S32, v, &v_min, &v_max, format, flags);
}
