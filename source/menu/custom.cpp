...
void CCustomUI::render_arrows_for_horizontal_bar(ImVec2 pos, float alpha, float width, float height) {
    auto draw = GetWindowDrawList();
    draw->AddRect(pos, pos + ImVec2(width, height), ImColor(0.f, 0.f, 0.f, alpha * GetStyle().Alpha), 4);
}

bool CCustomUI::expand_button(const char* label, bool selected, float rounding, ImDrawFlags flags) {
...
bool CCustomUI::tab(const char* icon, const char* label, bool selected) {
    ImGuiWindow* window = GetCurrentWindow();
    ImGuiID id = window->GetID(label);
...
bool CCustomUI::sub_tab(const char* label, bool selected) {
    ImGuiWindow* window = GetCurrentWindow();
    ImGuiID id = window->GetID(label);
...
void CCustomUI::tab_area(const char* str_id, ImVec2 size, std::function<void()> content) {
    ...
    value->second.hovered = IsMouseHoveringRect(GetCurrentWindow()->Pos, GetCurrentWindow()->Pos + ImVec2(value->second.width, size.y));
...
void CCustomUI::begin_child(const char* name, ImVec2 size) {
    ImGuiWindow* window = GetCurrentWindow();
    ImVec2 pos = window->DC.CursorPos;

    BeginChild(std::string(name).append(".main").c_str(), size, false, ImGuiWindowFlags_NoScrollbar);

    GetWindowDrawList()->AddRectFilled(pos, pos + size, to_vec4(28, 30, 36, m_anim), 4);
    GetWindowDrawList()->AddText(pos + ImVec2(10, 8), get_accent_color(m_anim), name, FindRenderedTextEnd(name));
